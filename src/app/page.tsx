...
function StatCard({
  ...
  return (
    <Link href={href} className="glass-card p-5 hover:border-zinc-600 transition-all group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              {overdueTasks.slice(0, 5).map((task) => (
                <div key={task.id} className="flex items-center gap-3 p-2 rounded-lg bg-zinc-900/50">
                  ...
                  <span className="text-xs text-red-400">{task.due_date}</span>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="space-y-2">
              ...
              {todayTasks.map((task) => (
                <div key={task.id} className="flex items-center gap-3 p-2 rounded-lg bg-zinc-900/50">
                  ...
                  <span className="text-sm flex-1 truncate">{task.title}</span>
                  {task.due_time && (
                    <span className="text-xs text-zinc-400">{task.due_time}</span>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="glass-card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-semibold">Gym This Week</h2>
          </div>
          <p className="text-3xl font-bold">{weekWorkouts.length}</p>
          <p className="text-xs text-zinc-500 mt-1">workouts completed</p>
        </div>
        ...
        <div className="glass-card p-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Link
              href="/tasks"
              className="p-3 rounded-lg bg-zinc-900 hover:bg-zinc-800 text-sm text-center transition-colors"
            >
              + New Task
            </Link>
            <Link
